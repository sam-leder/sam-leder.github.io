...
function calculateScores(answers) {
    let scores = {"Kyber": Math.floor(Math.random() * 100), "FrodoKEM": Math.floor(Math.random() * 100), "McEliece": Math.floor(Math.random() * 100)};
...
for (const scheme in scores) {
    div.innerHTML += `<p> ${scheme}: ${scores[scheme]}</p>`;
}
